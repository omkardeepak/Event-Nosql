...
const Login = () => {
  const [email, setEmail] = useState('');
      ...
      const handleSubmit = async (e) => {
        ...
        const response = await fetch('/api/userlogin', {  // Make sure your API path is correct (/api/)
          method: 'POST',
          headers: {
...
<svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" fill="currentColor" class="bi bi-person-fill" className="w-full justify-center flex" viewBox="0 0 16 16">
  ...
  <path d="M4 8a1 1 0 1 1-2 0 1 1 0 0 1 2 0"/>
          ...
          <input placeholder="password" type="password" className="rounded-md  text-black p-2 border-neutral-400 border-2" onChange={handlePasswordChange}></input>
          </div>
